...
class AuthorityManager: NSObject, CLLocationManagerDelegate {
    ...
    class func checkAudioStatus(controller:BaseViewController?){
        ...
        switch authorizate {
        case .denied:
            UIAlertController.shwoAlertControl(controller!, style: .alert, title: "请允许使用麦克风", message: "游戏需要麦克风权限", cancel: "取消", doneTitle: "确定", cancelAction: {
...
//            print()
        case .denied:
            UIAlertController.shwoAlertControl(controller!, style: .alert, title: "获取相机权限", message: "游戏需要麦克风权限", cancel: "取消", doneTitle: "确定", cancelAction: {
